 ...
 * @version $Id$
 */
public class PTRS_AST_LoopWalkFinderProcessor extends PTRS_AST_ProblemProcessor {
    ...
    public PTRS_AST_LoopWalkFinderProcessor(final Arguments args) {
        this.args = args;
                    ...
                    if (matcher != null) {
                        ...
                        for (final Entry<TRSTerm, BigFraction> entry : multDist.entrySet()) {
                            ...
                            for (int i = 0; i < leaf.getValue().getZ().size(); i++) {
                                ...
                                if (loopTerm.isLinear() || loopTerm.containsVariablesAsOften(leaf.getValue().getX().getVariablePositions())) {
            ...
            if (matcher != null) {
                ...
                for (final Entry<TRSTerm, BigFraction> entry : multDist.entrySet()) {
                    ...
                    for (int i = 0; i < leaf.x.getValue().getZ().size(); i++) {
                        BigFraction countAll = BigFraction.MINUS_ONE;
                        //Check if we are allowed to use all occurrences
                        if (loopTerm.isLinear() || loopTerm.containsVariablesAsOften(leaf.x.getValue().getX().getVariablePositions())) {
                            countAll = fraction.multiply(
    ...
    private int countMatches(final TRSTerm targetTerm, final TRSTerm baseLoopTerm, final TRSSubstitution repeatingSubstitution, final boolean onlyOrtho) {
        if (baseLoopTerm == null) {
            ...
            if (!onlyOrtho) { // Count All Occurrences
                return targetTerm.maxNO(baseLoopTerm) > 0 ? 1 : 0;
            } else { // Count Orthogonal
                return targetTerm.maxOO(baseLoopTerm) > 0 ? 1 : 0;
            }
        } else {
            return -1;
            ...
            //                }
            //            } else { // Count Orthogonal
            //            int maxOM = targetTerm.maxOM(baseLoopTerm, repeatingSubstitution);
